...
public class MissionStepTest {
    ...
    void 육단계() {
        ...
        // 내 예약 목록 조회
        List<MyReservationResponse> myReservations1 = RestAssured.given().log().all()
                .cookie("token", brownToken)
                .contentType(ContentType.JSON)
                .get("/reservations-mine")
                .then().log().all()
        ...
        System.out.println("이전 내 예약 목록 사이즈: " + myReservations1.size());
        ...
        System.out.println("이후 내 예약 목록 사이즈: " + myReservations.size());

        // 예약 대기 상태 확인
        String status = myReservations.stream()
                .filter(it -> Objects.equals(it.getReservationId(), waiting.id()))
