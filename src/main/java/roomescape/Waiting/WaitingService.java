...
public class WaitingService {
    ...
    public WaitingResponse save(LoginMember loginMember, WaitingRequest waitingRequest) {
        ...
        //TODO 예약 대기 순서 표시
        Integer waitingNumber = waitingRepository.countWaitingNumber(waiting.getId(), theme.getId(), time.getId(), waiting.getDate()) + 1;

        return new WaitingResponse(waiting.getId(), theme.getName(), waitingRequest.getDate(), time.getEvent_value(), waitingNumber);
    }
    ...
    public List<MyReservationResponse> findByMemberId(Long memberId){
        ...
        List<MyReservationResponse> responses = waitingRepository.findWaitingsWithRankByMemberId(memberId).stream()
                .map(it -> new MyReservationResponse(it.getWaiting().getId(),
                        ...
                        it.getWaiting().getTime().getEvent_value(), (it.getRank() + 1) + "번째 예약대기"))
                .toList();
